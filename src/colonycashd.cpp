...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/colonycash.conf are parsed in qt/colonycash.cpp's main()
    ...
    if (IsArgSet("-?") || IsArgSet("-h") ||  IsArgSet("-help") || IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  colonycashd [options]                     " + strprintf(_("Start %s Daemon"), _(PACKAGE_NAME)) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "colonycash:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in colonycashd anymore. Use the colonycash-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
        ...
        {
#if HAVE_DECL_DAEMON
            fprintf(stdout, "ColonyCash Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect colonycashd signal handlers
    noui_connect();
