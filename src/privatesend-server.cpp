...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all ColonyCash related functionality
...
/*
    ...
    allow endless transaction that would bloat ColonyCash and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all ColonyCash specific functionality
