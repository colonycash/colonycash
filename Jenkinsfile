...
// which are part of the ColonyCash repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="colonycash-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /colonycash-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "colonycashpay-colonycash/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from colonycashpay-colonycash/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'colonycashpay-colonycash/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from colonycashpay-colonycash/develop"
            }
          }

          if (hasCache) {
            sh "cd /colonycash-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /colonycash-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /colonycash-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /colonycash-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /colonycash-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /colonycash-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /colonycash-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
