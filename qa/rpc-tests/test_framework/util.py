...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "colonycash.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for colonycashd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if colonycashd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('colonycashd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        # Create cache directories, run colonycashds:
        for i in range(MAX_NODES):
            datadir=initialize_datadir(cachedir, i)
            args = [ os.getenv("BITCOIND", "colonycashd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: colonycashd started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in colonycash.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    """
    Start a colonycashd and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("BITCOIND", "colonycashd")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'colonycashd exited' in str(e) #node must have shutdown
            ...
            if expected_msg is None:
                assert_msg = "colonycashd should have exited with an error"
            else:
                assert_msg = "colonycashd should have exited with expected error " + expected_msg
            raise AssertionError(assert_msg)

def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple colonycashds, return RPC connections to them
    """
