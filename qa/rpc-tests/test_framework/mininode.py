...
"""ColonyCash P2P network half-a-node.
...
def colonycashhash(s):
    return colonycash_hash.getPoWHash(s)
...
# Objects that map to colonycashd objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(colonycashhash(r))
            self.hash = encode(colonycashhash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in colonycashd indicates these should be deserialized as blocks
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to ColonyCash Node: %s:%d' % (self.dstaddr, self.dstport))
