...
export BUILDER_IMAGE_NAME="colonycash-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/colonycash"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_CCASH_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
