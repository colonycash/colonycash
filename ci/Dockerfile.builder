...
RUN groupadd -g ${GROUP_ID} colonycash
RUN useradd -u ${USER_ID} -g colonycash -s /bin/bash -m -d /colonycash colonycash
...
RUN mkdir /colonycash-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /colonycash-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /colonycash-src
